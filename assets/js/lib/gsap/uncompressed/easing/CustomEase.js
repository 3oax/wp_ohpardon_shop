...
( _gsScope._gsQueue || ( _gsScope._gsQueue = [] ) ).push( () => {
	_gsScope._gsDefine( 'easing.CustomEase', ['easing.Ease'], ( Ease ) => {
		var _numbersExp = /(?:(-|-=|\+=)?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/ig,
			...
			_normalize = function ( values, height, originY ) { // takes all the points and translates/scales them so that the x starts at 0 and ends at 1.
				...
				var tx = Number( values[0] ) * -1,
					ty = -originY,
					l = values.length,
					sx = 1 / ( Number( values[l - 2] ) + tx ),
					sy = -height || ( Math.abs( Number( values[l - 1] ) - Number( values[1] ) ) < 0.01 * ( Number( values[l - 2] ) - Number( values[0] ) ) ? _findMinimum( values ) + ty : Number( values[l - 1] ) + ty ),
					i;
		...
		p.setData = function( data, config ) {
			...
			if ( Number( values[0] ) !== 0 || Number( values[l - 2] ) !== 1 ) {
			...
			for ( i = 2; i < l; i += 6 ) {
				...
				points.push( a1, a2 );
				_bezierToPoints( a1.x, a1.y, Number( values[i] ), Number( values[i + 1] ), Number( values[i + 2] ), Number( values[i + 3] ), a2.x, a2.y, 1 / ( precision * 200000 ), points, points.length - 1 );
			}
			l = points.length;
			for ( i = 0; i < l; i++ ) {
				point = points[i];
