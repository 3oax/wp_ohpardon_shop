...
( _gsScope._gsQueue || ( _gsScope._gsQueue = [] ) ).push( () => {
	var _DEG2RAD = Math.PI / 180,
		...
		_pathDataToBezier = function( d ) {
			var a = String( d ).replace( _scientific, ( m ) => {
					var n = Number( m );
			...
			for ( i = 0; i < elements; i++ ) {
				...
				// "C" (cubic bezier)
					...
					segment[l++] = relativeX + Number( a[i + 3] ); // note: "*1" is just a fast/short way to cast the value as a Number. WAAAY faster in Chrome, slightly slower in Firefox.
					segment[l++] = relativeY + Number( a[i + 4] );
					segment[l++] = relativeX += Number( a[i + 5] );
					segment[l++] = relativeY += Number( a[i + 6] );
					/*
				...
				// "S" (continuation of cubic bezier)
					...
					segment[l++] = relativeX += Number( a[i + 3] );
					segment[l++] = relativeY += Number( a[i + 4] );
					/*
					 *if (y === segment[l-1] && y === segment[l-3] && x === segment[l-2] && x === segment[l-4]) { //if all the values are the same, eliminate the waste.
					 *	segment.length = l = l-6;
				...
				// "Q" (quadratic bezier)
					...
					relativeX += Number( a[i + 3] );
					relativeY += Number( a[i + 4] );
					difX = x - relativeX;
					difY = y - relativeY;
					segment[l++] = relativeX + difX * 2 / 3;
					segment[l++] = relativeY + difY * 2 / 3;
				...
				// "A" (arc)
					...
					beziers = _arcToBeziers( relativeX, relativeY, Number( a[i + 1] ), Number( a[i + 2] ), Number( a[i + 3] ), Number( a[i + 4] ), Number( a[i + 5] ), ( isRelative ? relativeX : 0 ) + Number( a[i + 6] ), ( isRelative ? relativeY : 0 ) + Number( a[i + 7] ) );
					if ( beziers ) {
		...
		_convertToPath = function( e, swap ) {
			...
			if ( type === 'rect' ) {
				r = Number( e.getAttribute( 'rx' ) ) || 0;
				ry = Number( e.getAttribute( 'ry' ) ) || 0;
				x = Number( e.getAttribute( 'x' ) ) || 0;
				y = Number( e.getAttribute( 'y' ) ) || 0;
				w = ( Number( e.getAttribute( 'width' ) ) || 0 ) - r * 2;
				h = ( Number( e.getAttribute( 'height' ) ) || 0 ) - ry * 2;
				...
				if ( type === 'circle' ) {
					r = ry = Number( e.getAttribute( 'r' ) ) || 0;
					rycirc = r * circ;
				} else {
					r = Number( e.getAttribute( 'rx' ) ) || 0;
					ry = Number( e.getAttribute( 'ry' ) ) || 0;
				...
				x = Number( e.getAttribute( 'cx' ) ) || 0;
				y = Number( e.getAttribute( 'cy' ) ) || 0;
				rcirc = r * circ;
				...
				points = String( e.getAttribute( 'points' ) ).match( _numbersExp ) || [];
				x = points.shift();
				y = points.shift();
		...
		MorphSVGPlugin = _gsScope._gsDefine.plugin( {
			...
			init( target, value, tween, index ) {
				...
				if ( shape ) {
					...
					pt = this._addTween( target, 'setAttribute', String( target.getAttribute( p ) ), `${shape}`, 'morphSVG', false, p, typeof value.precompile === 'object' ? ( a ) => {
