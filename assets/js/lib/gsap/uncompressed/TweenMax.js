...
( _gsScope._gsQueue || ( _gsScope._gsQueue = [] ) ).push( () => {
	_gsScope._gsDefine( 'TweenMax', ['core.Animation', 'core.SimpleTimeline', 'TweenLite'], ( Animation, SimpleTimeline, TweenLite ) => {
	 ...
	 * ----------------------------------------------------------------
	 */
	_gsScope._gsDefine( 'TimelineLite', ['core.Animation', 'core.SimpleTimeline', 'TweenLite'], ( Animation, SimpleTimeline, TweenLite ) => {
	 ...
	 * ----------------------------------------------------------------
	 */
	_gsScope._gsDefine( 'TimelineMax', ['TimelineLite', 'TweenLite', 'easing.Ease'], ( TimelineLite, TweenLite, Ease ) => {
	...
	( function() {
		...
		BezierPlugin._cssRegister = function() {
			...
			_internals._registerComplexSpecialProp( 'bezier', {
				parser( t, e, prop, cssp, pt, plugin ) {
	 ...
	 * ----------------------------------------------------------------
	 */
	_gsScope._gsDefine( 'plugins.CSSPlugin', ['plugins.TweenPlugin', 'TweenLite'], ( TweenPlugin, TweenLite ) => {
		/** @constructor **/
		...
		var _numExp = /(?:\-|\.|\b)(\d|\.|e\-)+/g,
			...
			_internals = CSSPlugin._internals = { _specialProps }, // provides a hook to a few internal methods that we need to access from inside other plugins
			...
			_registerPluginProp = _internals._registerPluginProp = function( p ) {
				if ( ! _specialProps[p] ) {
					...
					_registerComplexSpecialProp( p, {
						parser( t, e, p, cssp, pt, plugin, vars ) {
				...
				if ( rec ) {
					...
					if ( tm.svg ) { // if we're supposed to apply transforms to the SVG element's "transform" attribute, make sure there aren't any CSS transforms applied or they'll override the attribute ones. Also clear the transform attribute if we're using CSS, just to be clean.
						if ( _useSVGTransformAttr && t.style[_transformProp] ) {
							TweenLite.delayedCall( 0.001, () => { // if we apply this right away (before anything has rendered), we risk there being no transforms for a brief moment and it also interferes with adjusting the transformOrigin in a tween with immediateRender:true (it'd try reading the matrix and it wouldn't have the appropriate data in place because we just removed it).
								_removeProp( t.style, _transformProp );
							} );
						} else if ( ! _useSVGTransformAttr && t.getAttribute( 'transform' ) ) {
							TweenLite.delayedCall( 0.001, () => {
		...
		_registerComplexSpecialProp( 'transform,scale,scaleX,scaleY,scaleZ,x,y,z,rotation,rotationX,rotationY,rotationZ,skewX,skewY,shortRotation,shortRotationX,shortRotationY,shortRotationZ,transformOrigin,svgOrigin,transformPerspective,directionalRotation,parseTransform,force3D,skewType,xPercent,yPercent,smoothOrigin', {
			parser( t, e, parsingProp, cssp, pt, plugin, vars ) {
	 ...
	 * ----------------------------------------------------------------
	 */
	_gsScope._gsDefine( 'easing.Back', ['easing.Ease'], ( Ease ) => {
		...
		_wrap(
			'Circ',
			_create( 'CircOut', ( p ) => Math.sqrt( 1 - ( p -= 1 ) * p ) ),
			_create( 'CircIn', ( p ) => -( Math.sqrt( 1 - p * p ) - 1 ) ),
			_create( 'CircInOut', ( p ) => ((p*=2) < 1 ? -0.5 * ( Math.sqrt( 1 - p * p ) - 1 ) : 0.5 * ( Math.sqrt( 1 - ( p -= 2 ) * p ) + 1 )) )
		);
		...
		_wrap(
			'Expo',
			_create( 'ExpoOut', ( p ) => 1 - Math.pow( 2, -10 * p ) ),
			_create( 'ExpoIn', ( p ) => Math.pow( 2, 10 * ( p - 1 ) ) - 0.001 ),
			_create( 'ExpoInOut', ( p ) => ((p *= 2) < 1 ? 0.5 * Math.pow( 2, 10 * ( p - 1 ) ) : 0.5 * ( 2 - Math.pow( 2, -10 * ( p - 1 ) ) )) )
		);

		// Sine
		_wrap(
			'Sine',
			_create( 'SineOut', ( p ) => Math.sin( p * _HALF_PI ) ),
			_create( 'SineIn', ( p ) => -Math.cos( p * _HALF_PI ) + 1 ),
			_create( 'SineInOut', ( p ) => -0.5 * ( Math.cos( Math.PI * p ) - 1 ) )
		);
...
( function( window, moduleName ) {
				...
				if ( missing === 0 && func ) {
					...
					if ( global ) {
						...
						if ( typeof module !== 'undefined' && module.exports ) { // node
							...
							define( ( window.GreenSockAMDPath ? `${window.GreenSockAMDPath}/` : '' ) + ns.split( '.' ).pop(), [], () => cl );
						}
					}
	...
	p._init = function() {
		...
		if ( v.easeParams instanceof Array && ease.config ) {
			this._ease = ease.config( ...v.easeParams );
		}
