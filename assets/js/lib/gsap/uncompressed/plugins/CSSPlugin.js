	...
	_gsScope._gsDefine( 'plugins.CSSPlugin', ['plugins.TweenPlugin', 'TweenLite'], ( TweenPlugin, TweenLite ) => {
		...
		var _numExp = /(?:\-|\.|\b)(\d|\.|e\-)+/g,
			...
			_parsePosition = function( v, recObj ) {
				...
				if ( x === 'center' || isNaN( parseFloat( x ) ) && String( x ).indexOf( '=' ) === -1 ) { // remember, the user could flip-flop the values and say "bottom center" or "center bottom", etc. "center" is ambiguous because it could be used to describe horizontal or vertical, hence the isNaN(). If there's an "=" sign in the value, it's relative.
					x = '50%';
				}
			...
			parser( t, e, parsingProp, cssp, pt, plugin, vars ) {
				...
				if ( orig || _supports3D && has3D && m1.zOrigin ) { // if anything 3D is happening and there's a transformOrigin with a z component that's non-zero, we must ensure that the transformOrigin's z-component is set to 0 so that we can manually do those calculations to get around Safari bugs. Even if the user didn't specifically define a "transformOrigin" in this particular tween (maybe they did it via css directly).
					if ( _transformProp ) {
						...
						orig = String( orig || _getStyle( t, p, _cs, false, '50% 50%' ) ); // cast as string to avoid errors
			...
			parser( t, e, p, cssp, pt, plugin ) {
				...
				for ( i = 0; i < props.length; i++ ) { // if we're dealing with percentages, we must convert things separately for the horizontal and vertical axis!
					...
					if ( rel ) {
						...
						es = es.substr( 2 );
						en *= parseFloat( es );
						esfx = es.substr( String( en ).length - ( en < 0 ? 1 : 0 ) ) || '';
					} else {
						en = parseFloat( es );
						esfx = es.substr( String( en ).length );
					}
					if ( esfx === '' ) {
		...
		p.parse = function( target, vars, pt, plugin ) {
			...
			for ( p in vars ) {
				...
				if ( sp ) {
					...
					this._tween._propLookup[p] = this._addTween.call( this._tween, target.style, 'setProperty', `${_getComputedStyle( target ).getPropertyValue( p )}`, String( es ), p, false, p );
					continue;
				} else {
