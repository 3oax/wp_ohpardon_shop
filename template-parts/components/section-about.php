  ...
  <?php if( !empty($section_about['video']) ): ?>
    ...
    <div class="mx-0 md:mx-1 relative">
      ...
      <?php if( !empty($video['button']) ): ?>
        <a class="btn glightbox no-barba" data-glightbox type="video"<?php if( !empty($video['button']['target']) ): ?> target="<?= $video['button']['target']; ?>"<?php endif; ?> style="position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%);" href="<?= $video['button']['url']; ?>"><?= $video['button']['title']; ?></a>
      <?php endif; ?>
    </div>
  <?php endif; ?>
